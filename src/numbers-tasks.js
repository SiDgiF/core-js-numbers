...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const deltaX = x2 - x1;
  const deltaY = y2 - y1;
  const distanse = Math.sqrt(deltaX ** 2 + deltaY ** 2);
  return distanse;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const dotVector = x1 * x2 + y1 * y2;
  // длины векторов
  const lenght1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lenght2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  // угол между двумя векторами
  const angle = Math.acos(dotVector / (lenght1 * lenght2));
  return angle;
}
...
function getParallelepipedDiagonal(a, b, c) {
  // const lengthAB = Math.sqrt(a * a + b * b);
...
function isPowerOfTwo(num) {
  ...
  // while для проверки
  // Если число делится на 2 без остатка, делим на 2 и продолжаем,
  // пока не получим нечетное число или 1.
  while (number % 2 === 0) {
    number /= 2;
  }
  // После окончания цикла - проверка равно ли число 1.
  // Если равно, значит число было степенью двойки - true.
  // если нет, - false.
  return number === 1;
}
...
function toExponential(number, fractionDigits) {
  // встроенный метод toExponential()
...
function isNumber(number) {
  return (!Number.isNaN(number) && Number.isFinite(number)) || number === 0;
}
